...
for it in range(its):
    ...
    for ind, p in enumerate(points):
        ...
        if it > 75:
            for i in range(15):
                ...
                alpha = (0.0015 if it > 100 else 0.3)
                linewidth[ind, near_ind] = max(alpha * min(1/(0.1*i**2+1) - 0.00/(i+1), 1) + (1-alpha)*linewidth[ind, near_ind], 0)
                '''
                if it > its-fadeout:
        ...
        if plot:
            for i in range(con):
                ...
                plt.plot([p[0], nep[0]], [p[1], nep[1]],
                         color='k',  # 'lightyellow',
                         linewidth=1, #max(min(10*(linewidth[ind, paint_ind]**(6)-1e-3), 1), 0), # 1*linewidth[ind, paint_ind],  # (min(1/dis[near_ind],1)),
                         alpha= 1, #max(min(10*(linewidth[ind, paint_ind]**(4)-1e-3), 1), 0)
                         )
                # '''
                dis.mask[near_ind] = True

        force_vec_1 = np.nan_to_num(
            (-0.08*np.cos((30*it/spe)*np.pi)-0.1)
            * np.divide(
                dis_vec,
        ...
        force_vec_2 = np.nan_to_num(
            (0.05*np.sin(31*it/spe*np.pi+0.3)+0.5)
            * np.divide(
                dis_vec,
    ...
    force = 1.3*force  + mv1*0.6*np.sin(20*it/spe*np.pi) # + 2*(np.random.rand(n_points, 2)-0.5)*(0.3-vel)
    ...
    vel = np.clip(vel + 0.1 * force - 0.15*vel, a_max=3, a_min=-3)
