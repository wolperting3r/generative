...
pbar = ProgressBar(widgets=widgets, maxval=(its-76))
for it in range(its):
    if it == 150:
        print('Plot')
        plot = True
    ...
    for ind, p in enumerate(points):
        ...
        if it > 40:
            for i in range(n_points):
                ...
                alpha = (0.017 if it > 100 else 0.1)  # 0.017  # 0.025
        ...
        force_vec_2 = np.nan_to_num(
            (0.5*np.sin((freq+1*freq/30)*it/spe*np.pi+0.3)+0.3)
            * np.divide(
                dis_vec,
    ...
    force = 0.8*force + mv1*0.7*np.sin(2/3*freq*it/spe*np.pi) + 1*(np.random.rand(n_points, 2)-0.5)*(0.3-vel)
    ...
    vel = np.clip(vel + 0.1 * force - 0.4*vel, a_max=3, a_min=-3)
