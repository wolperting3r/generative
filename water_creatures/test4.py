...
for it in range(its):
    ...
    for ind, p in enumerate(points):
        ...
        force_vec_1 = np.nan_to_num(
            (-0.02*np.cos((30*it/spe)*np.pi)-0.1)
            * np.divide(
                dis_vec,
        ...
        force_vec_2 = np.nan_to_num(
            (0.005*np.sin(31*it/spe*np.pi+0.3)+0.5)
            * np.divide(
                dis_vec,
    ...
    force = force #  + mv1*0.3*np.sin(20*it/spe*np.pi) # + 2*(np.random.rand(n_points, 2)-0.5)*(0.3-vel)
